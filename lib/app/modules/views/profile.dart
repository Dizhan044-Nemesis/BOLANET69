...
class _ProfileScreenState extends State<ProfileScreen> {
  ...
  void initState() {
    ...
    _firestoreService.syncLocalData();
  }
  ...
  Future<void> _pickMedia(bool isVideo, ImageSource source) async {
    ...
    if (selectedFile != null) {
      ...
      Map<String, dynamic> updateData = {
        'profile_image': _storedImagePath,
        'profile_video': _storedVideoPath,
      };
      await _firestoreService.uploadData(updateData, 'x3xm5nObS5aNHzF17O9H');
    }
  }
  ...
  Widget _buildProfileHeader() {
    return StreamBuilder<DocumentSnapshot>(
      stream: FirebaseFirestore.instance
          .collection('users')
          .doc('x3xm5nObS5aNHzF17O9H') // Replace with dynamic user ID
          .snapshots(),
...
class FirestoreService {
  ...
  Future<void> uploadData(Map<String, dynamic> data, String docId) async {
    try {
      final connectivityResult = await _connectivity.checkConnectivity();
      if (connectivityResult == ConnectivityResult.none) {
        _saveDataLocally(data, docId);
        return;
      }
      ...
      _removeLocalData(docId);
    } catch (e) {
      _saveDataLocally(data, docId);
    }
  }

  void _saveDataLocally(Map<String, dynamic> data, String docId) {
    final localData = _storage.read('local_data') ?? {};
    localData[docId] = data;
    _storage.write('local_data', localData);
  }

  void _removeLocalData(String docId) {
    final localData = _storage.read('local_data') ?? {};
    localData.remove(docId);
    _storage.write('local_data', localData);
  }
  ...
  Future<void> syncLocalData() async {
    ...
    if (localData.isNotEmpty) {
      for (String docId in localData.keys) {
        await uploadData(localData[docId], docId);
      }
    }
  }
}